...
def generate_summary(directory, level=0, ignoreReadMe=False, parentName=root_name):
    summary = ""
    indent = "  " * level
    ...
    for item in sorted(os.listdir(directory)):
        ...
        if os.path.isdir(item_path): # if item is a folder
            ...
            # else:
            summary += generate_summary(item_path, level, False, currentName)
            ...
            with open(item_path, "r") as file:
                for line in file:
                    ...
                    print(item_path)
                    print("No title found.")
            indent = "  " * (level-1) # To account for the folder that did not print
